...
class Nav2dEnv2(gym.Env):
    ...
    def __init__(self, debug=False):
        ...
        self.obs_low_state = np.array([-1, -1, -2, -2, 0]) # x_agent,y_agent, x_goal_agent, y_goal_agent, distance
        self.obs_high_state = np.array([1, 1, 2, 2, 1])
    ...
    def _observation(self):
        return np.array([self.agent_x, self.agent_y, self.goal_x_agent, self.goal_y_agent, self._distance()])

    def _normalize_observation(self, obs):
        normalized_obs = obs.copy()
        normalized_obs[:2] = normalized_obs[:2]/255*2 -1
        normalized_obs[2:4] = normalized_obs[2:4]/255*2
